...
/**
 * Shared landing layout rendered identically by the root home page and each
 * location home page. Both routes fetch the same `HomePageData` shape and hand
 * it to this component.
 */
export default function HomeLanding({ data, headerData }: { data: HomePageData; headerData: HeaderData }) {
  const timelineItems = data.home_experience_timeline_items.data.map((item) => ({
    imageUrl: item.attributes.homeExperienceTimelineImage.data!.attributes.url,
    imageAlt: item.attributes.homeExperienceTimelineImage.data!.attributes.alternativeText,
    imageWidth: item.attributes.homeExperienceTimelineImage.data!.attributes.width,
    imageHeight: item.attributes.homeExperienceTimelineImage.data!.attributes.height,
    title: item.attributes.homeExperienceTimelineTitle,
    ...
    <main>
      ...
      <section id="home-faq" className="px-[30px] xl:px-[100px] mt-[100px]">
        ...
        <ul className=" xl:grid xl:grid-rows-2 xl:grid-flow-col pb-[60px] xl:pb-0">
          {data.home_faqs.data.map((faq, index) => {
